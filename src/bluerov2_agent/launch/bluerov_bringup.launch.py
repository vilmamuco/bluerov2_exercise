...
def generate_launch_description():
    pkg_bluerov = get_package_share_directory('bluerov2_agent')
    ...
    hackathon_apm_config_path = os.path.join(
        pkg_bluerov, 'launch', 'apm_config.yaml')
    hackathon_apm_plugin_path = os.path.join(
        pkg_bluerov, 'launch', 'apm_pluginlists.yaml')
    ...
    agent_node = Node(
        package='bluerov2_agent',
        executable='bluerov2.py',
        output='screen'
    )
